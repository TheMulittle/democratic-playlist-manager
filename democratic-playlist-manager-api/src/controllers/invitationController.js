...
async function createInvitation(req, res) {
  ...
  const { inviteToken } = await invitationService.createInvitation(playlistId, playlistName, token);
...
async function getInvitation(req, res) {
  ...
  const invitation = await invitationService.getInvitation(inviteToken, playlistId);
...
async function acceptInvitation(req, res) {
  const token = req.headers.authorization?.split(" ")[1];
  const { inviteToken, playlistId } = req.params;
  await invitationService.acceptInvitation(inviteToken, playlistId, token);
